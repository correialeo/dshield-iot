...
void setup() {
  ...
  Serial.println("🔧 Use os potenciômetros para simular sensores");
  Serial.println("=====================================");
...
void simulateWaterSensor() {
  if (millis() - lastWaterReading > waterReadingInterval) {
    // ler potenciômetro e converter para nível de água (0-200cm)
    int potValue = analogRead(WATER_SENSOR_PIN);
    waterLevel = map(potValue, 0, 4095, 0, 200);
    ...
    Serial.printf("🌊 [WATER SENSOR] Nível: %.1f cm (ADC: %d)\n", waterLevel, potValue);
...
void simulateSmokeSensor() {
  if (millis() - lastSmokeReading > smokeReadingInterval) {
    // ler potenciômetro e converter para nível de fumaça (0-1000ppm)
    int potValue = analogRead(SMOKE_SENSOR_PIN);
    smokeLevel = map(potValue, 0, 4095, 5, 1000);
...
void simulateMeshMessage(uint32_t fromNodeId, int deviceId, float value, String sensorType) {
  // cria mensagem JSON como se viesse de outro nó
  ...
  // simula recebimento da mensagem (Gateway processando)
